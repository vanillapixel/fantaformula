...
return function(PDO $db) {
    ...
    $db->exec('CREATE TABLE IF NOT EXISTS __tmp_rr (id INTEGER PRIMARY KEY AUTOINCREMENT, race_id INTEGER NOT NULL, driver_id INTEGER NOT NULL, starting_position INTEGER, race_position INTEGER, fastest_lap BOOLEAN DEFAULT 0, dnf BOOLEAN DEFAULT 0, calculated_at DATETIME, FOREIGN KEY (race_id) REFERENCES races(id) ON DELETE CASCADE, FOREIGN KEY (driver_id) REFERENCES drivers(id) ON DELETE CASCADE, UNIQUE(race_id, driver_id))');
    if ($hasPos && !$hasRacePos) {
        $db->exec('INSERT INTO __tmp_rr (id,race_id,driver_id,starting_position,race_position,fastest_lap,dnf,calculated_at) SELECT id,race_id,driver_id,starting_position,position,fastest_lap,dnf,calculated_at FROM race_results');
    } else {
        $colsSelect = 'id,race_id,driver_id,starting_position,' . ($hasRacePos ? 'race_position':'position') . ',fastest_lap,dnf,calculated_at';
        $db->exec('INSERT INTO __tmp_rr (id,race_id,driver_id,starting_position,race_position,fastest_lap,dnf,calculated_at) SELECT ' . $colsSelect . ' FROM race_results');
    }
